...
fly_support = [
    simulate_fly(t, a, g, s) for t, a, g, s in zip(T, age, gender, ses)
]
...
fly_cpt = ConditionalProbabilityTable(
    cpt_entries,
    [treatment_dist, age_dist, gender_dist, ses_dist]
)
fly_node = Node(fly_cpt, name="fly_support")

# 4. Build the Bayesian Network
model = BayesianNetwork("fly Support BBN")
model.add_states(treatment_node, age_node, gender_node, ses_node, fly_node)

model.add_edge(treatment_node, fly_node)
model.add_edge(age_node, fly_node)
model.add_edge(gender_node, fly_node)
model.add_edge(ses_node, fly_node)
